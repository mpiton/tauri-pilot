...
The JS bridge monkey-patches the browser console methods, listens for `error` and `unhandledrejection`, and stores entries in a 500-entry ring buffer with timestamp, level, serialized arguments, and source location. The human-readable renderer prints `source` after the arguments when it is set. A console call or unhandled rejection raised directly by a script sent with `eval` reports `tauri-pilot-eval` as its source. A call inside an app function the script invokes keeps the app frame, and a callback the script schedules (`setTimeout`, `.then`) has no source; on WebKit (Linux, macOS) neither does a call made after an `await` in the script.
