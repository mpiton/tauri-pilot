...
The JS bridge monkey-patches the browser console methods, listens for `error` and `unhandledrejection`, and stores entries in a 500-entry ring buffer with timestamp, level, serialized arguments, and source location. The human-readable renderer prints `source` after the arguments when it is set. Entries produced by a script sent with `eval` report `tauri-pilot-eval` as their source.
