...
- `scroll top` and `scroll bottom` now actually scroll. Previously the bridge `scroll()` only computed dx/dy for `up`/`down`/`left`/`right`, so `top` and `bottom` returned `✓ ok` while `scrollBy(0, 0)` was a silent no-op. The bridge now uses `scrollTo(scrollX, 0)` for `top` and `scrollTo(scrollX, max(documentElement.scrollHeight, body.scrollHeight) - innerHeight)` for `bottom` on the window (preserves horizontal scroll, quirks-mode safe), and `scrollTop = 0` / `scrollTop = scrollHeight - clientHeight` on element refs. Unknown directions now throw instead of silently no-op (behavior change: scripts that previously got `{ ok: true }` for typos like `"Down"` will now get an error). The MCP `scroll` tool schema also exposes `top` and `bottom` in its direction enum, and `docs/reference/cli.md` lists them ([#73])
