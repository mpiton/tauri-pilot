...
//! `tauri-plugin-global-shortcut` handlers. The `global-hotkey` crate (which
//! is X11-only — Wayland is unsupported by the upstream crate) registers
//! shortcuts via `XGrabKey` passive grabs that match against the X server's
//! logical modifier state. `enigo`'s `XTestFakeKeyEvent` backend injects each
//! modifier and keycode as a separate fake-input call, and the modifier state
//! observed at the moment the main keycode is processed does not always match
//! the grab's exact-modifier-mask, so the grab may not activate. DOM
//! listeners and Tauri accelerators are unaffected and continue to receive
